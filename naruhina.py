...
# ----------------------------------------
async def check_partner_presence(
    update: Update,
...
async def start_duet_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_naruto = (bot1.token == BOT1_TOKEN)
...
async def pause_duet_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update, context):
        return
    grp = group_chats.get(chat_id)
    if grp and grp["chat_started"]:
        grp["paused"] = True
...
async def chat_loop(chat_id: int, bot1, bot2):
    ...
    while True:
        grp = group_chats.get(chat_id)
        if not grp or not grp["chat_started"]:
            break
        if grp.get("paused", False):
        ...
        speaker, text = Story[idx].split(":", 1)
